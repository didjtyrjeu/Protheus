...
//Vari�veis Est�ticas
Static cTitulo := "Gest�o de frota"

/*/{Protheus.doc} CLMKM
Fun��o para Gest�o de frota
@type function
...
Montagem do menu de op��es
@type function
@author Bryan Malheiros
@since 14/v4/2025
@version 1.0.0
@return array, Array com as op��es do menu
/*/
...
Static Function fCancelar()
	...
	Local nOperation  := If(oModel != NIL, oModel:GetOperation(), 0)

	/*'Pesquisar' 1 / 'Visualizar' 2 / 'Incluir' 3 / 'Alterar' 4 / 'Excluir' 5 / 'Cancelar' 6 / 'Conhecimento' 7 / 'Imprimir' 8 / 'Legenda' 9 */
	If nOperation == MODEL_OPERATION_CANCEL
		If Z17->Z17_STATUS # 'A'
			lRet := .F.
			Help( , , 'N�o Permitido' , , 'O registro n�o pode ser cancelado!', 1, 0, , , , , , {"Entre em contato com o setor de TI."})
		Else
			If MsgYesNo('Confirma o cancelamento do registro?')
				If Z17->(RecLock('Z17', .F.))
					//Altera o status sem abrir a tela de altera��o
					Z17->Z17_STATUS := 'C'
					Z17->(MsUnLock())
					MsgInfo('Registro cancelado com sucesso.')
				Else
					lRet := .F.
					Help( , , 'N�o Permitido' , , 'N�o foi poss�vel bloquear o registro para cancelamento.', 1, 0, , , , , , {"Entre em contato com o setor de TI."})
				EndIf
			EndIf

			//Impede que a rotina continue para a tela de altera��o
			lRet := .F.
		EndIf
	EndIf
...
Montagem da visualiza��o em MVC
@type function
@author Bryan Malheiros
@since 14/04/2025
@version 1.0.0
@return object, Objeto da visualiza��o
/*/
...
Fun��o para montar a estrutura do modelo de dados
@type function
@author Bryan Malheiros
@since 14/04/2025
@version 1.0.0
...
@author helder
@since 12/17/2023
@return variant, Retorna um valor l�gico True ou False
/*/
Static Function fAlterar( oModel )
	...
	if Z17->Z17_STATUS # 'A'
		If nOperation == 4 
			lRet := .F.
			Help( , , 'N�o Permitido' , , 'O registro n�o pode ser alterado!', 1, 0, , , , , , {"Entre em contato com o setor de TI."})
		Elseif nOperation == 6  
			lRet := .F.
			Help( , , 'N�o Permitido' , , 'O registro n�o pode ser cancelado.', 1, 0, , , , , , {"Entre em contato com o setor de TI."})
		EndIf
	...
	If nOperation == 5 .and. Z17->Z17_STATUS $ 'EC'
		lRet := .F.
		Help( , , 'N�o Permitido' , , 'O registro n�o pode ser exclu�do!', 1, 0, , , , , , {"Entre em contato com o setor de TI."})
	EndIf
	*/
...
Fun��o para exibir legenda de status dos equipamentos
@type function
@author Bryan Malheiros
@since 14/04/2025
@version 1.0
...
/*/{Protheus.doc} CLMAKM
Verifica se o chamada n�o est� Encerrada para alterar o conhecimento
@type function
@version 1.0.0 
@author Bryan Malheiros
@since 16/04/2025
...
Pre valida��o dos dados no Model, se pode Alterar,Cancelar ou Excluir Chamado
@type function
@version 1.0.0 
@author helder
@since 12/17/2023
...
Static Function zVlPre()
	...
	If nOperation == 4
		If Z17->Z17_STATUS == 'E' //.and. !(cUserId $ cUsersID)
			...
			lRet := .F.
			Help( , , 'N�o Permitido' , , 'O registro j� est� encerrado.', 1, 0, , , , , , {"Entre em contato com o setor de TI."})
	
		Elseif Z17->Z17_STATUS == 'C' 
			lRet := .F.
			Help( , , 'N�o Permitido' , , 'O registro j� est� cancelado.', 1, 0, , , , , , {"Entre em contato com o setor de TI."})
	...
	If nOperation == 5
		If Z17->Z17_STATUS $ 'E' //.and. !(cUserId $ cUsersID)
			...
			lRet := .F.
			Help( , , 'N�o Permitido' , , 'A gest�o da frota n�o pode ser excluida.', 1, 0, , , , , , {"Entre em contato com o setor de TI."})
		EndIf
	EndIf
	*/
...
/*/{Protheus.doc} zVlPos
P�s valida��o dos dados no Model, valida��o de patrim�nio
@type function
@version 1.0.0
@author helder
@since 12/17/2023
...
Static Function zVlPos()
	...
	If nOperation == 3 .or. nOperation == 9
		...
		If MsSeek(xFilial("Z17") + cNum)
			lRet := .F.
			Help( , , 'N�o Permitido' , , 'C�digo de sequencia j� cadastrado!', 1, 0, , , , , , {"Entre em contato com o setor de TI."})
		EndIf
	EndIf
